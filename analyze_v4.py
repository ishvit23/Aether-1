...
try:
    ...
    hunts = sum(
        1 for e in events if e.get("event") == "hunt" and e.get("payload", {}).get("success")
    )  # noqa: E501
    attacks = sum(
        1 for e in events if e.get("event") == "attack" and e.get("payload", {}).get("success")
    )  # noqa: E501
    builds = sum(
        1
        for e in events
        if e.get("event") == "build" and e.get("payload", {}).get("success") is not False
    )  # noqa: E501
    births = sum(
        1 for e in events if e.get("event") == "reproduce" and e.get("payload", {}).get("success")
    )  # noqa: E501
